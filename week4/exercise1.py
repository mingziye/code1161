...
def get_some_details():
    ...
    return {"lastname":     data["results"][0]["name"]["last"],
            ...
            "postcodPlusId":  int(data["results"][0]["location"]["postcode"]) + int(data["results"][0]["id"]["value"])
...
def wordy_pyramid():
    ...
    import requests
    base = "http://api.wordnik.com/v4/words.json/randomWords?api_key=a2a73e7b926c924fad7001ca3111acd55af2ffabf50eb4ae5&minLength=10&maxLength=10&limit=1"
    pyramid = []
    for x in range (3,21,2):
        url = base.format(x)
        req = requests.get(url)
        message = req.json()[0]["word"]
